...
async function downloadImage(url, filepath) {
  return new Promise((resolve) => {
    const thumUrl = `https://image.thum.io/get/width/600/crop/400/${url}`;
...
async function processProjects() {
  ...
  const allProjects = [...shopifyProjects, ...wpProjects, ...laravelProjects];
  ...
  for (let i = 0; i < allProjects.length; i++) {
    ...
    console.log(`Downloading ${i+1}/${allProjects.length}: ${p.url}`);
    await downloadImage(p.url, filepath);
  }
    ...
    return projects.map(p => `
      <div class="col-md-4 mb-4 project-card" data-category="${p.cat}" data-tech="${p.tech}">
        <div class="card h-100 shadow-sm border-0">
          <img src="${p.img}" class="card-img-top border-bottom" alt="${p.title}" loading="lazy">
          <div class="card-body d-flex flex-column">
            ...
            <div class="mt-auto d-flex gap-2">
              ...
              <a href="${p.url}" target="_blank" class="btn btn-sm btn-primary w-50">Visit</a>
            </div>
          </div>
        </div>
      </div>
  ...
  console.log('HTML files regenerated with local screenshots.');
}
